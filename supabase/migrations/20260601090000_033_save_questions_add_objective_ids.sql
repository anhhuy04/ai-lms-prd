-- 033_save_questions_add_objective_ids.sql
-- Rủi ro #3: AI-generate "Lưu & thêm vào Đề thi" làm MẤT objective mastery analytics.
--
-- save_questions_to_assignment (bản 028) khi snapshot câu AI-generate thành câu
-- INLINE (question_id = NULL) chỉ lưu vào custom_content: type, override_text,
-- choices, expected_answer, ai_grading_keywords, blanks, general_explanation.
-- NHƯNG KHÔNG lưu 'objective_ids'.
--
-- Trigger analytics (032 — fn_update_skill_mastery / fn_recalculate_skill_mastery)
-- với câu inline (question_id IS NULL) chỉ có thể đọc objective từ
-- custom_content->'objective_ids' (hoặc 'learningObjectives'). Vì snapshot thiếu
-- field này nên câu AI thêm vào đề KHÔNG đóng góp student_skill_mastery.
--
-- FIX: CREATE OR REPLACE function với chữ ký Y HỆT bản 028, sao chép NGUYÊN VẸN
-- toàn bộ logic (vẫn tạo bank private, vẫn dedup ON CONFLICT, vẫn mirror choices,
-- vẫn snapshot inline question_id=NULL), CHỈ BỔ SUNG 'objective_ids' vào snapshot.
--
-- Nguồn objective_ids (UNION 2 nguồn để forward-correct):
--   (a) v_q->'objective_ids'  — payload câu AI (hoạt động NGAY khi client gửi field này).
--   (b) question_objectives của v_qid — phủ trường hợp ON CONFLICT khi v_qid trỏ tới
--       bank question CŨ đã có objective gắn sẵn (RPC không tự insert question_objectives,
--       nhưng v_qid có thể là row đã tồn tại).
-- Giá trị xuất ra là JSON array các chuỗi uuid-text — đúng contract trigger 032 đọc.
--
-- ⚠️ CẢNH BÁO (client fix vẫn cần): hiện QuestionDTO.toDbInsert() (question_dto.dart)
-- và staging_area_widget.dart KHÔNG gửi objective_ids/learningObjectives trong payload
-- mỗi câu. Do đó với câu AI MỚI (chưa có bank question trùng), migration này tạo
-- objective_ids = [] (vô hại nhưng chưa fix analytics). Migration là CẦN nhưng CHƯA ĐỦ:
-- chỉ thực sự fix khi client bổ sung 'objective_ids' vào payload toDbInsert().
...
BEGIN
  ...
  LOOP
    ...
    IF p_assignment_id IS NOT NULL THEN
      ...
      -- 033: gom objective_ids cho câu inline (question_id NULL) để trigger
      -- analytics đọc được từ custom_content->'objective_ids'. UNION 2 nguồn:
      -- (a) payload v_q->'objective_ids', (b) question_objectives của v_qid (bank cũ).
      SELECT COALESCE(jsonb_agg(DISTINCT oid), '[]'::jsonb)
        INTO v_obj_ids
      FROM (
        SELECT jsonb_array_elements_text(
                 CASE WHEN jsonb_typeof(v_q->'objective_ids') = 'array'
                      THEN v_q->'objective_ids'
                      ELSE '[]'::jsonb END
               ) AS oid
        UNION
        SELECT qo.objective_id::text AS oid
        FROM public.question_objectives qo
        WHERE qo.question_id = v_qid
      ) s
